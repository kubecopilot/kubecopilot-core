...
KIND_CLUSTER ?= kube-copilot-agent-test-e2e
...
# (i.e. container build --platform linux/arm64). However, you must enable buildKit for it.
...
container-build-agent: ## Build agent container image
	$(CONTAINER_TOOL) build -t $(AGENT_IMG) ./agent-server-container/github-copilot/

.PHONY: container-push-agent
container-push-agent: ## Push agent container image
	$(CONTAINER_TOOL) push $(AGENT_IMG)

UI_IMG ?= quay.io/gfontana/kube-copilot-agent-ui:v1.0

.PHONY: container-build-ui
container-build-ui: ## Build web UI container image
	$(CONTAINER_TOOL) build -t $(UI_IMG) -f ./web-ui/Containerfile ./web-ui/

.PHONY: container-push-ui
...
container-buildx: ## Build and push container image for the manager for cross-platform support
	...
	- $(CONTAINER_TOOL) buildx create --name kube-copilot-agent-builder
	$(CONTAINER_TOOL) buildx use kube-copilot-agent-builder
	- $(CONTAINER_TOOL) buildx build --push --platform=$(PLATFORMS) --tag ${IMG} -f Containerfile.cross .
	- $(CONTAINER_TOOL) buildx rm kube-copilot-agent-builder
