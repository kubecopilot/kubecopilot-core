...
func main() {
	...
	mgr, err := ctrl.NewManager(ctrl.GetConfigOrDie(), ctrl.Options{
		...
		LeaderElectionID:       "430949b1.kubecopilot.io",
	...
	if err := (&controller.KubeCopilotMessageReconciler{
		Client: mgr.GetClient(),
		Scheme: mgr.GetScheme(),
	}).SetupWithManager(mgr); err != nil {
		setupLog.Error(err, "Failed to create controller", "controller", "KubeCopilotMessage")
		os.Exit(1)
	}
	if err := (&controller.KubeCopilotSendReconciler{
		Client: mgr.GetClient(),
		Scheme: mgr.GetScheme(),
	}).SetupWithManager(mgr); err != nil {
		setupLog.Error(err, "Failed to create controller", "controller", "KubeCopilotSend")
		os.Exit(1)
	}
	if err := (&controller.KubeCopilotCancelReconciler{
		Client: mgr.GetClient(),
		Scheme: mgr.GetScheme(),
	}).SetupWithManager(mgr); err != nil {
	...
	if err := (&controller.KubeCopilotPolicyReconciler{
		Client: mgr.GetClient(),
		Scheme: mgr.GetScheme(),
	}).SetupWithManager(mgr); err != nil {
	...
	// Start the agent-response webhook server alongside the manager.
	webhookSrv := agentwebhook.New(mgr.GetClient(), ":8090")
	go func() {
